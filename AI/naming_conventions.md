...
* **Правило:** Имя должно точно отражать назначение сущности. Запрещены бессмысленные, шутливые или дезинформирующие имена.
* **Правило:** Все имена должны быть на английском языке, с соблюдением грамматики (например, `horizontalAlignment`, а не `alignmentHorizontal`).
* **Правило:** Имена должны быть произносимыми и легко доступными для поиска (избегать коротких имен, таких как `$d` или `$ds`).
* **Правило:** Использовать симметричные пары для противоположных операций: `begin/end`, `lock/unlock`, `min/max`.

## Классы

* **Формат:** PascalCase
* **Тип слова:** Существительное.
* **Правило:** Имя класса описывает объект или сущность. Избегать "шумных" слов, таких как `Data`, `Info`, `Object`.
* **Пример:** ✅ `Customer`, `InvoiceParser` / ❌ `CustomerData`, `ParserObject`
* **Правило:** Если класс реализует известный паттерн проектирования, отразить это в его имени.
    * **Пример:** `CustomerBuilder`, `OrderFactory`, `PaymentStrategy`
