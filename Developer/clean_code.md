...
### 3. Repositories (и альтернативы)

*   **Что это?**
    Классически, репозитории — это классы, которые инкапсулируют логику запросов к базе данных, выступая посредником между бизнес-логикой и хранилищем данных.

*   **Проблема в Laravel:**
    Часто в Laravel этот паттерн является **избыточным**. Eloquent, будучи реализацией паттерна Active Record, уже предоставляет мощный и выразительный API для работы с БД. "Репозитории" в Laravel-проектах часто вырождаются в простые хелперы, дублирующие функциональность **Query Scopes**.

*   **Когда же это может быть полезно?**
    *   Если проект очень большой и есть реальная потребность абстрагироваться от Eloquent (например, для возможного перехода на другую ORM).
    *   Если необходимо работать с несколькими источниками данных (БД, внешние API) под единым интерфейсом.

*   **Альтернатива: Query Objects или специализированные классы-запросы**
    Вместо полноценного репозитория для повторяющихся сложных запросов лучше использовать **Query Scopes** в моделях или создавать небольшие, сфокусированные классы (например, `GetAvailableProductsByCategoryQuery`), которые инкапсулируют только эту конкретную логику. Это позволяет избежать создания лишнего слоя абстракции.

*   **Решение для проекта:**
    В нашем проекте мы **не используем** паттерн Repository в его классическом виде. Сложные и повторяющиеся запросы следует выносить в **Query Scopes** на соответствующих моделях.
...
Статические методы могут быть полезны, но их неправильное использование приводит к трудно тестируемому и связанному коду.

### Когда статические методы — это ХОРОШО?

Используйте их для **"stateless" (без состояния) хелперов или фабричных методов**. Такой метод не зависит от состояния объекта или внешних зависимостей. Он всегда даёт одинаковый результат для одинаковых входных данных.

*   **Примеры:**
    *   Фабричные методы на DTO: `OrderData::fromRequest($request)`.
    *   Простые хелперы: `Str::slug($title)`, `Carbon::now()`.
    *   Методы, которые инкапсулируют простое создание объекта без сложных зависимостей.

```php
// Хороший пример: stateless-хелпер
class CurrencyConverter
{
    public static function fromRubToUsd(float $amount): float
    {
        // Логика конвертации без зависимостей
        return $amount / 95.5;
    }
}
```

### Когда статические методы — это ПЛОХО?

**Избегайте статики для основной бизнес-логики**, особенно если она зависит от других классов или имеет состояние.

*   **Причины:**
    1.  **Скрытые зависимости:** `MyService::process()` может внутри себя вызывать другие статические методы или использовать фасады, что делает зависимости неявными.
    2.  **Трудности с тестированием:** Статические методы трудно "мокать" (mock). Вы не можете легко подменить `UserService::create($data)` в тесте, чтобы он не лез в базу данных. Вам придётся использовать сложные инструменты вроде Mockery.
    3.  **Нарушение инверсии зависимостей:** Вы привязываетесь к конкретной реализации, а не к абстракции (интерфейсу).

*   **Пример:**

```php
// Плохо: бизнес-логика в статическом методе
class OrderService
{
    public static function create(User $user, array $items)
    {
        // ... какая-то логика
        $order = Order::create(...); // Скрытая зависимость от Eloquent
        Notification::send(...); // Скрытая зависимость от фасада
        return $order;
    }
}

// Хорошо: использование обычного метода и внедрение зависимостей
class OrderService
{
    public function __construct(private PaymentGateway $gateway) {}

    public function create(User $user, array $items)
    {
        // ... какая-то логика
        $this->gateway->charge(...);
        $order = new Order(...);
...
**Основное правило:** если метод для своей работы нуждается в других сервисах или классах, он **не должен** быть статическим. Вместо этого внедряйте зависимости через конструктор.
...
Коллекции Laravel предоставляют мощный и выразительный API для работы с массивами данных. Они делают код более читаемым и лаконичным по сравнению с нативными PHP-функциями.
...
### Не размещайте логику в Blade-шаблонах

Шаблоны должны быть максимально "глупыми" и отвечать только за отображение данных, которые подготовил контроллер или модель.

*   **❌ Плохо (запрос к БД в шаблоне):**
    ```blade
    {{ App\Models\User::find(1)->name }}
    ```
*   **✅ Хорошо (данные переданы в шаблон):**
    ```blade
    {{ $user->name }}
    ```
